...
public class MazeView extends DefaultViewer {
	...
	void redrawFinish(Graphics gc) {
		...
		FontMetrics fm = gc.getFontMetrics();
		centerString(gc, fm, "You won!", 120);
		gc.setFont(smallBannerFont);
		gc.setColor(Color.white);
		fm = gc.getFontMetrics();
		centerString(gc, fm, "Path length is " +
		Integer.toString(controller.driver.getPathLength()), 45);
		gc.setFont(smallBannerFont);
		gc.setColor(Color.red);
		fm = gc.getFontMetrics();
		centerString(gc, fm, "Battery left is " +
		Float.toString(controller.robot.getBatteryLevel()), 140);
		gc.setColor(Color.yellow);
		gc.setFont(smallBannerFont);
		fm = gc.getFontMetrics();
		centerString(gc, fm, "Congratulations!", 180);
		// write the instructions
		gc.setColor(Color.white);
